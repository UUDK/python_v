...
class Race(t.NamedTuple):
    name: str
    horses: list[Horse]
    status: dict[str, RaceStatus]  # "status"
    winner: dict[str, Horse | None]  # "winner"


def add_horse(race: Race, horse_name: str, odds: float):
    race.horses.append(Horse(name=horse_name, owner="", rating=odds))


def find_horse(race: Race, horse_name: str):
